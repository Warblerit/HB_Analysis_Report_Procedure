...
GO
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[SP_HBAnalysis_Test]') AND OBJECTPROPERTY(ID, N'ISPROCEDURE') = 1)
DROP PROCEDURE [dbo].[SP_HBAnalysis_Test]
GO 
CREATE PROCEDURE [dbo].[SP_HBAnalysis_Test](@Action NVARCHAR(100),@Str NVARCHAR(100),@Id BIGINT)
--Exec [dbo].[SP_HBAnalysis_Test] @Action = '',@Str = '',@Id = 0;
AS
BEGIN
SET NOCOUNT ON
