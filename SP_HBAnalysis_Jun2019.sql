...
GO
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[SP_HBAnalysis_Jun2019]') AND OBJECTPROPERTY(ID, N'ISPROCEDURE') = 1)
DROP PROCEDURE [dbo].[SP_HBAnalysis_Jun2019]
GO 
CREATE PROCEDURE [dbo].[SP_HBAnalysis_Jun2019](@Action NVARCHAR(100),@Str NVARCHAR(100),@Id BIGINT)
/*
Exec [dbo].[SP_HBAnalysis_Jun2019] @Action = '',@Str = '',@Id = 0;
*/
AS
BEGIN
SET NOCOUNT ON
...
[Apr-2019] INT, [May-2019] INT, [Jun-2019] INT);
...
ISNULL([Apr-2019], 0), ISNULL([May-2019], 0), ISNULL([Jun-2019], 0)
FROM #output;
...
SUM([Apr-2019]), SUM([May-2019]), SUM([Jun-2019])
...
[Apr-2019] DECIMAL(27,2), [May-2019] DECIMAL(27,2), [Jun-2019] DECIMAL(27,2));
...
ROUND([Apr-2019] * 0.00001, 2), ROUND([May-2019] * 0.00001, 2), ROUND([Jun-2019] * 0.00001, 2)
...
ROUND(SUM([Apr-2019]) * 0.00001, 2), ROUND(SUM([May-2019]) * 0.00001, 2), ROUND(SUM([Jun-2019]) * 0.00001, 2)
...
SELECT '' AS C1,
'Jun-19' AS C15, 'May-19' AS C15, 'Apr-19' AS C14,
...
[Mar-2019], [Feb-2019], [Jan-2019], [Dec-2018], [Nov-2018], [Oct-2018], [Sep-2018], [Aug-2018], [Jul-2018], [Jun-2018], [May-2018], [Apr-2018]
FROM #TDRClient T
ORDER BY Flg ASC, [Jun-2019] DESC, MasterClientName ASC;
...
[Mar-2019], [Feb-2019], [Jan-2019], [Dec-2018], [Nov-2018], [Oct-2018], [Sep-2018], [Aug-2018], [Jul-2018], [Jun-2018], [May-2018], [Apr-2018]
FROM #GTVTOTALCLIENT
ORDER BY MarkupFlg ASC, [Jun-2019] DESC, MasterClientName ASC;
...
SUM(ISNULL([Apr-2019],0)), SUM(ISNULL([May-2019],0)), SUM(ISNULL([Jun-2019],0))
FROM #TEMPMONTH12
WHERE MarkUp != 0;
...
ROUND(SUM([Apr-2019]) * 0.00001, 0), ROUND(SUM([May-2019]) * 0.00001, 0), ROUND(SUM([Jun-2019]) * 0.00001, 0)
FROM #TEMPMONTH16;
...
[Dec-2018] INT, [Jan-2019] INT, [Feb-2019] INT, [Mar-2019] INT, [Apr-2019] INT, [May-2019] INT, [Jun-2019] INT, Flg INT);
...
SELECT '' AS C1,
'Jun-19' AS C16, 'May-19' AS C15, 'Apr-19' AS C14,
...
WHERE T.MasterClientId IN (SELECT MasterClientId FROM HBAnalysis_ClientList WHERE Active = 1)
GROUP BY T.MasterClientId, M.ClientName
ORDER BY SUM(ISNULL([Jun-2019],0)) DESC;
