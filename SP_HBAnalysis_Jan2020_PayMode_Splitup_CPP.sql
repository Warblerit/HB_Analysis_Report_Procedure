...
GO
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[SP_HBAnalysis_Jan2020_PayMode_Splitup_CPP]') AND OBJECTPROPERTY(ID, N'ISPROCEDURE') = 1)
DROP PROCEDURE [dbo].[SP_HBAnalysis_Jan2020_PayMode_Splitup_CPP]
GO 
CREATE PROCEDURE [dbo].[SP_HBAnalysis_Jan2020_PayMode_Splitup_CPP](@Action NVARCHAR(100),@Str NVARCHAR(100),@Id BIGINT)
/*
Exec [dbo].[SP_HBAnalysis_Jan2020_PayMode_Splitup_CPP] @Action = '',@Str = '',@Id = 0;
*/
AS
BEGIN
SET NOCOUNT ON
...
WHERE
S.Status != 'Canceled' AND S.PropertyCategory = 'C P P'
