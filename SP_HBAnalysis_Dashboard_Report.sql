...
GO
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[SP_HBAnalysis_Dashboard_Report]') AND OBJECTPROPERTY(ID, N'ISPROCEDURE') = 1)
DROP PROCEDURE [dbo].[SP_HBAnalysis_Dashboard_Report]
GO 
CREATE PROCEDURE [dbo].[SP_HBAnalysis_Dashboard_Report](@Action NVARCHAR(100),@Str NVARCHAR(100),@Id BIGINT)
/*
Exec [dbo].[SP_HBAnalysis_Dashboard_Report] @Action = '',@Str = '',@Id = 0;
*/
AS
BEGIN
SET NOCOUNT ON
...
SELECT * FROM #TEMP3;RETURN;

DELETE #TEMP2 WHERE asasd NOT IN ('Mar-2019', 'Apr-2019', 'May-2019', 'Jun-2019', 'Jul-2019', 'Aug-2019');
...
ROUND(SUM([Aug-2019]) * 0.00001, 2), ROUND(SUM([Sep-2019]) * 0.00001, 2)
...
FROM #TDR T
ORDER BY Flg ASC, [Sep-2019] DESC, MasterPropertyName ASC;
...
[Mar-2019], [Feb-2019], [Jan-2019], [Dec-2018], [Nov-2018], [Oct-2018], [Sep-2018], [Aug-2018], [Jul-2018], [Jun-2018], [May-2018], [Apr-2018]
FROM #TDRClient T
ORDER BY Flg ASC, [Sep-2019] DESC, MasterClientName ASC;
...
[Mar-2019], [Feb-2019], [Jan-2019], [Dec-2018], [Nov-2018], [Oct-2018], [Sep-2018], [Aug-2018], [Jul-2018], [Jun-2018], [May-2018], [Apr-2018]
FROM #GTVTOTALCLIENT
ORDER BY MarkupFlg ASC, [Sep-2019] DESC, MasterClientName ASC;
...
ROUND(SUM([Aug-2019]) * 0.00001, 0), ROUND(SUM([Sep-2019]) * 0.00001, 0)
FROM #TEMPMONTH16;
...
[Dec-2018] INT, [Jan-2019] INT, [Feb-2019] INT, [Mar-2019] INT, [Apr-2019] INT, [May-2019] INT, [Jun-2019] INT, [Jul-2019] INT, [Aug-2019] INT, [Sep-2019] INT, Flg INT);
...
WHERE T.MasterClientId IN (SELECT MasterClientId FROM HBAnalysis_ClientList WHERE Active = 1)
GROUP BY T.MasterClientId, M.ClientName
ORDER BY SUM(ISNULL([Sep-2019],0)) DESC;
